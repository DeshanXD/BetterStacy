...
module.exports.run = async (bot, message, args) => {
    ...
    let userObject = bot.users.cache.get(userId)


    const votemuteEmbed = new MessageEmbed()
        .setColor('#0099ff')
        .setAuthor(`${userObject.username}`, `${userObject.avatarURL()}`)
        .setDescription(`${userObject} You are getting vote muted by people on ${message.member.voice} initiated by ${message.author} `);
    ...
    try {
        ...
        if (userId === message.author.id) {
            ...
            await message.reply('You need to be in voice channel in order to run this command!')
        } else {
...
module.exports.help= {
    name: "servermute",
    desc: " Server mute random people!"
}